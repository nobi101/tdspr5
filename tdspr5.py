...
hong = "\033[1;95m"
# Đánh dấu bản quyền
edit = trang + "~" + red + "[" + vang + "⟨⟩" + red + "] " + trang + "➩ "
edit1 = trang + "~" + red + "[" + luc + "✓" + red + "] " + trang + "➩ "
#####[THƯ VIỆN]#######
import os, sys
try:
...
def LuuCookie():
	while True:
		if os.path.exists('Page-Fb-tds.txt'):
			...
			chon = input(edit+'VUI LÒNG NHẬP LỰA CHỌN: '+trang)
			if chon == '1':
				with open('Page-Fb-tds.txt', 'r') as f:
					list = json.loads(f.read())
					break
			elif chon == '2':
				...
				while True:
					...
					if "1" in nhiemvu:
						try:
							...
							print(xnhac+" ĐÃ TÌM THẤY "+vang+str(len(listnvlike))+xnhac+" NHIỆM VỤ LIKE",end = '\r');sleep(1); print('                                                        ', end = '\r')
							for D in listnvlike:
								id = D["id"]
					...
					if "2" in nhiemvu:
						try:
							...
							print(xnhac+" ĐÃ TÌM THẤY "+vang+str(len(listnvlike))+xnhac+" NHIỆM VỤ LIKE 2",end = '\r');sleep(1); print('                                                        ', end = '\r')
							for D in listnvlike:
								id = D["id"]
								if "_" in id:idlike = id.split("_")[1]
								else:idlike = id
					...
					if "3" in nhiemvu:
						try:
							...
							print(xnhac+" ĐÃ TÌM THẤY "+vang+str(len(listnvcx))+xnhac+" NHIỆM VỤ REACTION",end = '\r');sleep(1); print('                                                        ', end = '\r')
							for D in listnvcx:
								id = D["id"]
								type = D["type"]
								if "_" in id:idlike = id.split("_")[1]
					...
					if "4" in nhiemvu:
						try:
							...
							print(xnhac+" ĐÃ TÌM THẤY "+vang+str(len(listnvshare))+xnhac+" NHIỆM VỤ SHARE",end = '\r');sleep(1); print('                                                        ', end = '\r')
					...
					if "5" in nhiemvu:
						try:
							...
							print(xnhac+" ĐÃ TÌM THẤY "+vang+str(len(listnvcmt))+xnhac+" NHIỆM VỤ CMT",end = '\r');sleep(1); print('                                                        ', end = '\r')
							for D in listnvcmt:
								id = D["id"]
								msg = D["msg"]
					...
					if "6" in nhiemvu:
						try:
							...
							print(xnhac+" ĐÃ TÌM THẤY "+vang+str(len(listnvgr))+xnhac+" NHIỆM VỤ GROUP",end = '\r');sleep(1); print('                                                        ', end = '\r')
							for D in listnvgr:
								id = D["id"]
					...
					if "7" in nhiemvu:
						try:
							...
							print(xnhac+" ĐÃ TÌM THẤY "+vang+str(len(listnvpage))+xnhac+" NHIỆM VỤ LIKE PAGE",end = '\r');sleep(1); print('                                                        ', end = '\r')
							for D in listnvpage:
								id = D["id"]
